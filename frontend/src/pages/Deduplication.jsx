        ...
        <tr className="border-t" style={{ borderColor: "var(--border)", background: "var(--panel)" }}>
          ...
          <td colSpan={4} className="px-2 py-3">
            ...
            <div className="flex items-center gap-4 text-[13px]" style={{ color: "var(--muted)" }}>
              ...
              <span>{a.duplicate_count} identical firings collapsed into 1</span>
              <span className="font-mono" style={{ color: "var(--muted-dim, var(--muted))" }}>fp: {a.fingerprint}</span>
            </div>
          </td>
        </tr>
      )}
  ...
  return (
    <div className="p-6 overflow-auto h-full">
      ...
      <p className="text-[14px] mb-4" style={{ color: "var(--muted)" }}>
        ...
        An alert counts as a repeat of another when it shares the same service, alert type, and fires within a 5-minute window.
      </p>
      ...
      <div className="grid grid-cols-2 min-[1280px]:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard icon={<FingerprintPattern size={16} />} label="Unique After Dedup" value={stats.unique_count} color="var(--accent)" delta="after deduplication" />
        ...
        <StatCard icon={<TrendingDown size={16} />} label="Reduction" value={`${stats.reduction_pct}%`} color="var(--ok)" delta="fewer alerts to review" />
      </div>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: "var(--border)", background: "var(--panel)" }}>
        <div className="px-4 py-3 text-[15px] font-semibold border-b flex items-center gap-2" style={{ borderColor: "var(--border)" }}>
          ...
          Groups where repeat alerts got collapsed into one
        ...
        <table className="w-full text-[15px]">
          <thead>
            <tr className="text-left text-[13px] uppercase tracking-wider" style={{ color: "var(--muted)" }}>
              <th className="px-4 py-2 font-medium w-8">Sev</th>
